...
//    }
  $countsql = "select filename from (select filename, count(distinct(workerid)) as freq from buildnetlabelinfo where label != 'Array' and workerid in (select id from userinfo where percent >= 70) group by filename) as T where T.freq >= 5;"; 
//echo($countsql);
            if(strlen($countsql) > 0) {
                $result = $conn->query($countsql);
                
                while($row = $result->fetch_assoc()) {
                    $fname = $row["filename"];
                    $hit5filename[] = trim($fname); 
                }
            }
...
//
   ...
   // Write initial hits data for shapes if the file does not exist
    ...
    foreach ($hits as $key => $val) {
      ...
      if(in_array(trim($key), $hit5filename)) {
          $hits[$key] = 150001;
          $val  = 150001;
      }
...
<div class="w3-main" id="main" style=" left:float:left;display:inline-block; position:relative; width:100%; margin 0 auto;">
  ...
  <div class="w3-row" id="modelcontainer" style="position:relative; position:inline-block;  max-height: 1600px; height:70vh" >
    ...
    <span id=">=70% complete" style="background-color:black; display:inline-block; color:white;position:absolute; left:5%; top:5%">>=70% valid: 0.5$ reward</span>
    <span id=">=80% complete" style="background-color:black; display:inline-block; color:white;position:absolute; left:5%; top:8%">>=80% valid: bonus +0.25$=0.75$</span>
    <span id=">=90% complete" style="background-color:black; display:inline-block; color:white;position:absolute; left:5%; top:11%">>=90% valid: bonus +0.5$=1.0$</span>
    ...
    <div style="width:36%; float:right; border-style:groove">
      <div id="labeling" style=" padding-top:0.01%; padding-right:5%">
        ...
        <form action="rewritehit.php" method="post" id="rewritehitform" name="rewritehitform">
             ...
             <input type="hidden" id="rw_userid" name="rw_userid" value=<?php echo($_SESSION["userid"])?>>
