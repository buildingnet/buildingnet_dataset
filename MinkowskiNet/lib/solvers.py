...
from torch.optim.lr_scheduler import LambdaLR, StepLR, ReduceLROnPlateau, MultiStepLR, CosineAnnealingLR
...
def initialize_optimizer(params, config):
    ...
    if config.optimizer == "SGD":
        ...
        logging.error(f"Optimizer type {config.optimizer} not supported")
        raise ValueError(f"Optimizer type {config.optimizer} not supported")


def initialize_scheduler(optimizer, config, last_step=-1, factor=0.5, patience=10, cooldown=0, milestones=None,
                         mlt_stp_gamma=0.1, min_lr=0):
    if config.scheduler == "StepLR":
        ...
        return ReduceLROnPlateau(optimizer, patience=patience, cooldown=cooldown, factor=factor)
    elif config.scheduler == "MultiStepLR":
        return MultiStepLR(optimizer, milestones=milestones, gamma=mlt_stp_gamma, last_epoch=last_step)
    elif config.scheduler == "CosineAnnealingLR":
        return CosineAnnealingLR(optimizer, T_max=config.max_iter, eta_min=min_lr, last_epoch=last_step)
    else:
        logging.error(f"Scheduler {config.scheduler} not supported")
